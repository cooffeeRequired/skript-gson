    ...
    private static JsonElement jsonMainTree(@NotNull String name, boolean isLocal, boolean nullable, Event event) {
        ...
        if (variable.keySet().stream().filter(Objects::nonNull).allMatch(SimpleUtil::isNumeric)) {
            ...
            if (isIncrementNumber(checkKeys.toArray())) {
                ...
                keys.forEach(key -> {
                    JsonElement valueData = gsonAdapter.toJsonTree(JsonAdapter.toJson(jsonListSubTree(name + key, isLocal, event)));
                    if (valueData instanceof JsonPrimitive) {
                        JsonPrimitive primitive = valueData.getAsJsonPrimitive();
                        jsonStructure.add(fromString2JsonElement(fromPrimitive2Object(primitive)));
                    } else {
                ...
                keys.forEach(key -> {
                    JsonElement data = gsonAdapter.toJsonTree(JsonAdapter.toJson(jsonListSubTree(name+key, isLocal, event)));
                    if (data instanceof JsonPrimitive) {
                        JsonPrimitive primitive = (JsonPrimitive) data;
                        jsonStructure.add(key, fromString2JsonElement(fromPrimitive2Object(primitive)));
                    } else {
    ...
    private static void extractNestedObjects(@NotNull String variableName, @NotNull JsonElement input, boolean isLocal, Event event) {
        if (input instanceof JsonObject) {
            input.getAsJsonObject().keySet().forEach(key -> {
                if (!(key == null)) jsonToList(variableName + SEPARATOR + key, input.getAsJsonObject().get(key), isLocal, event);
            });
        } else if (input instanceof JsonArray) {
            for (int index = 0; input.getAsJsonArray().size() > index; index++) jsonToList(variableName + SEPARATOR + (index + 1), input.getAsJsonArray().get(index), isLocal, event);
        }
    }
}
